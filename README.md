...
- **Authenticated private rooms:** Leaving the room field blank generates a high-entropy `bpsr5` invitation that pins the host's Ed25519 public key. Every participant has a separate signing identity, host-only commands cannot be forged by another room member, and replayed packets are rejected. Multiplayer Ready status is bound to the exact shared MIDI/conversion revision, so older clients cannot silently join an incompatible room.
...
- **Per-part entrances:** The host can set a **Start at** song time beside every assigned lobby part. Everyone keeps the same synchronized song clock, while that part stays silent before its entrance and joins at the authored position afterward. Changing an entrance makes the affected remote player confirm Ready again.
...
- **Double single-note melody (octave):** Piano-only option that thickens a lone melody attack with the same note one octave lower, or one octave higher when the lower copy falls outside the selected Range. With Musical roles grouping it only doubles the detected Melody part; otherwise the entire onset must contain one note. Existing octave pairs, full chords, `Max chord notes: 1`, and ranges narrower than an octave are left unchanged.
