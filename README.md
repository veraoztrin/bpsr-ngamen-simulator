...
- **Authenticated private rooms:** Leaving the room field blank generates a high-entropy `bpsr4` invitation that pins the host's Ed25519 public key. Every participant has a separate signing identity, host-only commands cannot be forged by another room member, and replayed packets are rejected. Multiplayer Ready status is bound to the exact shared MIDI/conversion revision, so older `bpsr3` clients cannot silently join an incompatible room.
